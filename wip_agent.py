...
if "GOOGLE_API_KEY" not in os.environ:
    os.environ["GOOGLE_API_KEY"] = "AIzaSyDtNZWy9zQmJjmW-F2iwSR_xFHYtAt_eGs"

# Initialize the NEW Client (v1beta/v2 unified)
client = genai.Client(api_key=os.environ["GOOGLE_API_KEY"])

# ⚠️ CONFIRMED: Valid as of Nov 18, 2025
MODEL_NAME = "gemini-3-pro-preview"

# LangChain instance (We keep this for the text summary generation)
...
# ==========================================
# 3. THE AGENTS (New SDK Implementation)
# ==========================================

def extractor_node(state: WipState):
    print(f"\n---  GEMINI 3 PRO IS READING (NEW SDK): {state.file_path} ---")
    ...
    try:
        # NEW SDK CALL PATTERN (Solves the "Hang")
        ...
        # New SDK returns a Parsed Object if schema is provided, 
        # but to be safe across versions, we load the text.
        if hasattr(response, "parsed") and response.parsed:
             # If the SDK auto-parsed it into our Pydantic model
            raw_rows = response.parsed.rows
        else:
...
def analyst_node(state: WipState):
    ...
    # Using LangChain wrapper for text is fine, or use client.models.generate_content(..., config=types.GenerateContentConfig(response_mime_type="text/plain"))
    narrative = llm.invoke(f"Summary. Total Contract: ${t_contract}. GP: {gp_pct:.1f}%. Risks: {len(risks)} jobs.").content
